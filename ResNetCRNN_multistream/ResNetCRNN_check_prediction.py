...
# set path
data_path = {'spatial': '/scratch/ielnaqa_fluxg/thuanhsi/UCF101/jpegs_256/',
             'motion_x': '/scratch/ielnaqa_fluxg/thuanhsi/UCF101/tvl1_flow/u/',
             'motion_y': '/scratch/ielnaqa_fluxg/thuanhsi/UCF101/tvl1_flow/v/'}   # UCF-101 data path
action_name_path = './UCF101actions.pkl'
frame_slice_file = './UCF101_frame_count.pkl'
save_model_path = "/scratch/ielnaqa_fluxg/thuanhsi/Video_Classification/model_ckpt/multistream/"
...
all_data_loader = data.DataLoader(Dataset_CRNN_multistream(data_path, all_X_list, all_y_list, selected_frames, transform=transform), **all_data_params)


# create multistream CRNN model
...
# reload multistream CRNN model
spatial_encoder.load_state_dict(torch.load(os.path.join(save_model_path, 'spatial_encoder_epoch41.pth')))
motion_encoder.load_state_dict(torch.load(os.path.join(save_model_path, 'motion_encoder_epoch41.pth')))
rnn_decoder.load_state_dict(torch.load(os.path.join(save_model_path, 'rnn_decoder_epoch41.pth')))
print('Multistream CRNN model reloaded!')

# make all video predictions by reloaded model
print('Predicting all {} videos:'.format(len(all_data_loader.dataset)))
all_y_pred = CRNN_final_prediction_multistream([spatial_encoder, motion_encoder, rnn_decoder], device, all_data_loader)
